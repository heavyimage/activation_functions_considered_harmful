...
# =============================================================================
"""simple network to learn multiplication
"""
import math
import sys
import yaml
...
def filename_generator(extension, quantized):
    ...
    model_name = "multiplication_with_{af}{q}{p}.{ext}".format(
            af="sigmoid",
            q=qstring,
            p=p,
            ext=extension)
...
def get_data():
    """
    The code will generate a set of random pairs (x) and their multiplication (y)
    """
    a = np.random.uniform(low=-10, high=10, size=100000).astype(np.float32)
    b = np.random.uniform(low=-10, high=10, size=100000).astype(np.float32)
    x_values = np.dstack((a,b))[0]

    # Shuffle the values to guarantee they're not in order
    np.random.shuffle(x_values)

    # Calculate the corresponding multiplication value
    y_values = np.multiply(x_values[:,1:], x_values[:,:1])
...
def create_model() -> tf.keras.Model:
    ...
    model.add(tf.keras.layers.Dense(4, activation="sigmoid", input_shape=(2, )))
